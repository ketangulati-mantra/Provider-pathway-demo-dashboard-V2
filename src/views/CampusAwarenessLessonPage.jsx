...
import { goToDashboard, goToLesson } from '../mantra';
import { GraduationCap, Award, Heart, Check, Sparkles, HelpCircle } from 'lucide-react';
...
export default function CampusAwarenessLessonPage({ onBack }) {
  const handleYesInterested = () => {
    // DO NOT mark completed here. Navigate to educational module.
    goToLesson('/task/campus-ambassador-learning');
  };

  const handleNotRightNow = () => {
    // DO NOT mark completed or award points. Simply redirect to dashboard.
    goToDashboard();
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg-app)' }} className="animate-fade-in">
      <Header title={LESSON_TITLE} onBack={onBack} progress={0} points={50} />
